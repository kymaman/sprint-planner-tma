...
await test('Заметка + подзадачи: добавление в модалке, бейдж 1/2', async () => {
  await page.click(tid('task-edit-goal-0-task-2'));
  await page.waitForSelector(tid('task-modal'));
  await page.fill(tid('task-note-input'), 'Заметка E2E');
  await page.fill(tid('subtask-add-input'), 'Саб один');
  await page.click(tid('subtask-add-btn'));
  await page.fill(tid('subtask-add-input'), 'Саб два');
  await page.click(tid('subtask-add-btn'));
  await page.click(tid('subtask-toggle-0'));
  await page.click(tid('task-save-btn'));
  await page.waitForSelector(tid('task-modal'), { state: 'detached' });
  await page.waitForSelector(tid('subtask-badge-goal-0-task-2'));
  const badge = await page.locator(tid('subtask-badge-goal-0-task-2')).innerText();
  if (!badge.includes('1/2')) throw new Error('бейдж: ' + badge);
});

await test('Заметка + подзадачи: переживают повторное открытие', async () => {
  await page.click(tid('task-edit-goal-0-task-2'));
  await page.waitForSelector(tid('task-modal'));
  const note = await page.locator(tid('task-note-input')).inputValue();
  if (note !== 'Заметка E2E') throw new Error('note: ' + note);
  const subs = await page.locator('[data-testid^="subtask-toggle-"]').count();
  if (subs !== 2) throw new Error('подзадач: ' + subs);
  await page.click(tid('task-save-btn'));
  await page.waitForSelector(tid('task-modal'), { state: 'detached' });
});
...
await test('Week: drag&drop — перенос Пн→Вт мышью за хэндл', async () => {
  const handles = await page.locator('[data-testid^="drag-"]').count();
  if (handles < 1) throw new Error('нет drag-хэндлов');
  const id = await page.evaluate(() => {
    const s = JSON.parse(localStorage.getItem('sprint'));
    for (const g of s.goals) for (const t of g.tasks) if (t.title === 'Новая задача E2E') return t.id;
    return null;
  });
  if (!id) throw new Error('задача «Новая задача E2E» не найдена');
  const hb = await page.locator(tid('drag-' + id)).boundingBox();
  await page.mouse.move(hb.x + hb.width / 2, hb.y + hb.height / 2);
  await page.mouse.down();
  await page.mouse.move(hb.x + hb.width / 2 + 14, hb.y + hb.height / 2 + 14, { steps: 5 }); // >6px → drag активен
  ...
  const day = await page.evaluate((taskId) => {
    const s = JSON.parse(localStorage.getItem('sprint'));
    for (const g of s.goals) for (const t of g.tasks) if (t.id === taskId) return t.day;
  }, id);
  if (day !== 1) throw new Error('day после drop: ' + day);
});

await test('Review: burndown-график отрисован', async () => {
  await page.click('.tab-bar a:has-text("Обзор")');
  await page.waitForSelector(tid('burndown-chart'));
  const txt = await page.locator(tid('burndown-chart')).innerText();
  if (!txt.includes('Осталось')) throw new Error('нет счётчика остатка: ' + txt.slice(0, 60));
});
...
await test('Архив: сброшенный спринт попал в sprintArchive', async () => {
  const ok = await page.evaluate(() => {
    const a = JSON.parse(localStorage.getItem('sprintArchive') || '[]');
    return a.length >= 1 && a.some(s => s.title === 'Sprint E2E');
  });
  if (!ok) throw new Error('архив пуст или без Sprint E2E');
});
...
await test('Архив: restore двойным тапом меняет спринт местами с архивным', async () => {
  // сеем архивный спринт напрямую (SettingsPage читает его при маунте)
  await page2.evaluate(() => {
    const arch = {
      id: 'sprint-arch', title: 'Архивный спринт', startDate: '2026-01-05',
      goals: [0, 1, 2].map(i => ({
        id: 'ag-' + i, color: '#3ee0ff', title: 'Арх цель ' + (i + 1),
        habit: { id: 'ah-' + i, title: 'Арх привычка ' + (i + 1), checks: {} },
        tasks: [{ id: `ag-${i}-t0`, goalId: 'ag-' + i, title: 'Арх задача', week: 1, done: i === 0, isMain: false, estimatedDays: 1 }],
      })),
      weekNotes: {}, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    };
    localStorage.setItem('sprintArchive', JSON.stringify([arch]));
  });
  await page2.click('.tab-bar a:has-text("Спринт")');
  await page2.waitForSelector(tid('open-settings'));
  await page2.click(tid('open-settings'));
  await page2.waitForSelector(tid('archive-section'));
  await page2.waitForSelector(tid('archive-item-sprint-arch'));
  await page2.click(tid('archive-restore-sprint-arch'));      // 1-й тап — подтверждение
  await page2.click(tid('archive-restore-sprint-arch'));      // 2-й тап — restore
  await page2.waitForTimeout(400);
  const res = await page2.evaluate(() => {
    const s = JSON.parse(localStorage.getItem('sprint'));
    const a = JSON.parse(localStorage.getItem('sprintArchive') || '[]');
    return { cur: s?.title, archTitles: a.map(x => x.title) };
  });
  if (res.cur !== 'Архивный спринт') throw new Error('текущий: ' + res.cur);
  if (!res.archTitles.includes('E2E Sprint')) throw new Error('прежний не в архиве: ' + JSON.stringify(res.archTitles));
});
